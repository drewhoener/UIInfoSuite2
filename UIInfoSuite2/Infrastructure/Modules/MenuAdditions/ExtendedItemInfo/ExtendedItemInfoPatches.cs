...
internal partial class ExtendedItemInfoModule
{
  /***
  ...
  public void Patch(Harmony harmony)
  {
    harmony.Patch(
      ...
      postfix: new HarmonyMethod(typeof(ExtendedItemInfoModule), nameof(Item_getDescriptionWidth_Postfix))
    );
  ...
  /// <summary>
  ///   The width of a hovered item's textbox is determined by the item name (among other things).
  ...
  private static void Item_getDescriptionWidth_Postfix(ref int __result, Item __instance)
  {
    var bundleHelper = ModEntry.GetSingleton<BundleHelper>();
  ...
  {
    return new CodeMatch(instruction => instruction.opcode == code &&
  ...
  )
  {
    ...
    {
      ModEntry.Instance.Monitor.Log($"Failed to patch drawHoverText. Message: {e.Message}");
      return null;
    }
  ...
  )
  {
    ...
    matcher.MatchStartForward(
      ...
      // textureBoxDimensionModifiers = AddOffsetsIfBundleItem(hoveredItem, HeightOffset, startingHeight)
      .Insert(
        ...
        new CodeInstruction(
          OpCodes.Call,
          AccessTools.DeclaredMethod(typeof(ExtendedItemInfoModule), nameof(AddOffsetsIfBundleItem))
        ),
  ...
  private static void PatchDrawUIIconsHook(CodeMatcher matcher, LocalBuilder boxDrawYPos)
  {
    ...
    matcher.Advance(1)
      .Insert(
        ...
        new CodeInstruction(
          OpCodes.Call,
          AccessTools.DeclaredMethod(typeof(ExtendedItemInfoModule), nameof(DrawBundlesAndIcons))
        )
      );
  }

#region Patch Helpers and Hooks
  private enum ContainerPatchPoint
  {
    BeforeTitle,
    AfterTitle,
    AfterCategory,
  ...
  private static readonly Dictionary<ContainerPatchPoint, LayoutContainer> _additionalDrawContainers = new();
